...
(my/assert-key my/leader-map "." #'project-find-file)
(my/assert-key my/leader-map "*" #'my/search-symbol-at-point)
(my/assert-key my/leader-map "/" #'consult-line)
(my/assert-key my/leader-map "," #'project-switch-to-buffer)
(my/assert-unbound my/leader-map "<")
(my/assert-key my/leader-map "'" #'evil-show-marks)
...
(my/assert-key my/leader-map "j" #'evil-show-jumps)
...
(dolist (key '("B" "R" "i" "k"))
  (my/assert-unbound my/leader-buffers-map key))
...
(dolist (entry '(("-" . my/org-browse-notes)
                 ("a" . org-agenda)
...
(dolist (key '("C" "F" "o" "v"))
  (my/assert-unbound my/leader-notes-map key))
...
(dolist (entry '(("*" . my/search-symbol-at-point)
                 ("'" . evil-show-marks)
...
(dolist (key '("`" "i" "p" "u"))
  (my/assert-unbound my/leader-search-map key))
...
(let* ((backend (org-export-get-backend 'gfm))
       (actions (nth 2 (org-export-backend-menu backend)))
       (clipboard-actions (cl-remove-if-not
                           (lambda (entry) (eq (car entry) ?c))
                           actions)))
  (unless (and (= (length clipboard-actions) 1)
               (eq (nth 2 (car clipboard-actions))
                   #'my/org-gfm-export-to-clipboard))
    (error "Expected one GFM clipboard export action, got %S"
           clipboard-actions))
  (my/org-gfm-add-clipboard-export)
  (unless (= (cl-count ?c
                       (nth 2 (org-export-backend-menu backend))
                       :key #'car)
             1)
    (error "GFM clipboard export action was duplicated")))
(let (clipboard-text)
  (cl-letf (((symbol-function 'kill-new)
             (lambda (text &optional _replace) (setq clipboard-text text))))
    (with-temp-buffer
      (org-mode)
      (insert "* Heading\nText with *bold* and a [[https://example.com][link]].\n")
      (my/org-gfm-export-to-clipboard nil nil nil nil)))
  (unless (and (string-match-p "^# Heading" clipboard-text)
               (string-match-p "\\*\\*bold\\*\\*" clipboard-text)
               (string-match-p "\\[link\\](https://example.com)" clipboard-text))
    (error "Unexpected GFM clipboard export: %S" clipboard-text)))
(let (clipboard-text)
  (cl-letf (((symbol-function 'kill-new)
             (lambda (text &optional _replace) (setq clipboard-text text))))
    (with-temp-buffer
      ...
      (activate-mark)
      (my/org-gfm-export-to-clipboard nil nil nil nil)))
  (unless (and (string-match-p "Selected text" clipboard-text)
               (not (string-match-p "Excluded text" clipboard-text)))
    (error "GFM region export used the wrong scope: %S" clipboard-text)))
(condition-case nil
    (progn
      (my/org-gfm-export-to-clipboard t nil nil nil)
      (error "Expected asynchronous clipboard export to fail"))
  (user-error nil))
...
(dolist (entry '(("\\ t" . org-todo)
                 ("C-<return>" . my/org-insert-heading-respect-content)
                 ("M-<return>" . my/org-meta-return)
                 ("C-M-<return>" . my/org-insert-subheading)
                 ("S-<return>" . my/org-table-copy-down)
                 ("C-S-<return>" . my/org-insert-todo-heading-respect-content)
                 ...
                 ("z C" . outline-hide-subtree)
                 ("z m" . my/org-hide-next-fold-level)
                 ("z M" . org-overview)
                 ("z n" . org-tree-to-indirect-buffer)
                 ("z o" . my/org-open-fold)
                 ("z O" . org-fold-show-subtree)
                 ("z r" . my/org-show-next-fold-level)
