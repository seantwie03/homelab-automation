  ...
  (let (events)
    (cl-letf (((symbol-function 'magit-toplevel)
              ...
              ((symbol-function 'tab-bar-new-tab)
               (lambda () (push '(new-tab) events)))
              ((symbol-function 'tab-bar-rename-tab)
               (lambda (name) (push (list 'rename-tab name) events)))
              ...
              ((symbol-function 'magit-status)
               (lambda (directory &optional _cache)
                 (push (list 'magit-status directory) events))))
      (my/magit-status-new-tab)
      (should (equal (nreverse events)
                     '((new-tab)
                       (rename-tab "Magit")
                       (magit-status "/tmp/project/")
                       (delete-other-windows)))))))

(ert-deftest my/magit-status-new-tab-rejects-non-git-directory ()
  (cl-letf (((symbol-function 'magit-toplevel)
             (lambda (&optional _directory) nil)))
    (should-error (my/magit-status-new-tab) :type 'user-error)))
